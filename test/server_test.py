...
sandbox_controller = SandboxController(
    "_sandbox",
    sample_runner_path=Path(__file__).resolve().parent / ".." / "src" / "leety" / "server" / "exercise" / "internal_templates" / "exercise_sample_runner.py",
    sample_lib_path=Path(__file__).resolve().parent / ".." / "src" / "leety" / "server" / "exercise" / "base_generator.py",
    solution_runner_path=Path(__file__).resolve().parent / ".." / "src" / "leety" / "server" / "exercise" / "internal_templates" / "exercise_solution_runner.py",
)
exercise_controller = ExerciseController(database, user_controller, sandbox_controller)
solution_controller = SolutionController(database, sandbox_controller, exercise_controller)
valid_generator = exercise_controller.create_exercise(admin_user.id, ExerciseModel(
